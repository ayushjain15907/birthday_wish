...
function floatingProfile(){
    ...
    const validImages = profileImages.filter(
        src => src && src.trim() !== ""
    );
    ...
    img.onerror = function(){
        ...
        // Broken image ko screen par kabhi add nahi karna
        img.remove();
...
// 💥 Moving photo bubble click effect

document.addEventListener("click", function (e) {

    // Check if the clicked element is a moving photo bubble
    const bubble = e.target.closest(".floatingProfile");

    // If it is not a moving photo bubble, do nothing
    if (!bubble) {
        return;
    }

    // Prevent double clicking
    if (bubble.classList.contains("bubble-pop")) {
        return;
    }

    // Get the exact image currently inside this bubble
    const imageSrc = bubble.src;
    ...
    preview.className = "floating-photo-preview";
    ...
    // Remove large image after 1 second
    setTimeout(function () {
